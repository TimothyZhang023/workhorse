...
import { Modal, Select, Button, Spin, Empty, Tooltip, Tag, Divider } from 'antd';
import { ThunderboltOutlined, StopOutlined, CopyOutlined } from '@ant-design/icons';
            ...
            selectedModels.map(async (modelId, idx) => {
                try {
                    ...
                    const response = await fetch(`/api/conversations/${convId}/chat`, {
                        ...
                        body: JSON.stringify({ message: prompt, model: modelId, _compare: true }),
                        ...
                        for (const line of lines) {
                            ...
                            try {
                                ...
                                if (parsed.content) {
                                    setResults(prev =>
                                        prev.map((r, i) =>
                                            i === idx ? { ...r, content: r.content + parsed.content, loading: false } : r
                                        )
                                    );
                                }
                                if (parsed.error) {
                                    setResults(prev =>
                                        prev.map((r, i) =>
                                            i === idx ? { ...r, error: parsed.error, loading: false } : r
                                        )
                                    );
                                }
                            } catch (_) { }
    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                {running ? (
                    ...
                    <Button
                        ...
                        onClick={handleRun}
                        disabled={selectedModels.length < 2 || !prompt.trim() || !conversationId}
                    >
                        开始对比
                    </Button>
                )}
            ...
            {selectedModels.length < 2 && (
                <div style={{ color: '#9ca3af', fontSize: 13, textAlign: 'center', marginBottom: 12 }}>
                    请至少选择 2 个模型
                </div>
            )}
                            ...
                            >
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <Tag color="blue" style={{ margin: 0 }}>{r.displayName}</Tag>
                                ...
                                <div style={{ padding: '12px 16px', minHeight: 120 }}>
                                    {r.error ? (
                                        <span style={{ color: '#ef4444', fontSize: 13 }}>❌ {r.error}</span>
                                    ) : r.content ? (
                                        <MarkdownRenderer content={r.content} isDark={isDark} />
                                    ) : r.loading ? (
                                        <div style={{ color: '#9ca3af', fontSize: 13 }}>生成中...</div>
                                    ) : (
